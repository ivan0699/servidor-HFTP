...
class Server(object):
    ...
    def serve(self):
        ...
        while True:
            try:
                ...
                t = Thread(target=self.user_connection, args=(cli_sock, cli_addr))
                t.setDaemon(True)
                t.start()
                #print ("Connection from %s\n" % cli_addr[0])
                #new_connection = Connection(cli_sock, DEFAULT_DIR)
    ...
    def user_connection(self, *args):
        ...
        new_connection = Connection(connection, DEFAULT_DIR)
        new_connection.handle()
